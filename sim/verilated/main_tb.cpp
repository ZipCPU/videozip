...
//		master verilog module, main.v.  This module is designed to be
//	complete code sets implementing all of the functionality of the Digilent
//	Nexys-Video board---save for some hardware dependent features such as
//	the DDR3 memory.  If done well, any programs talking to this one should be
...
#define	ram_mem		v__DOT__blkram__DOT__mem
...
#define	console_setup	__DOT__console__DOT__uart_setup
#endif
...
public:
	...
	void	load(uint32_t addr, const char *buf, uint32_t len) {
		...
		if ((addr >= MEMBASE)&&(addr + len <= MEMBASE+MEMLEN)) {
			...
			memcpy(&m_core->ram_mem[addr-MEMBASE],
				bswapd, len);
			delete[] bswapd;
		}
#else
...
#else
		m_core->i_mdio = (*m_mid)(0, m_core->o_mdclk,
				((m_core->o_mdwe)&&(!m_core->o_mdio))?0:1);
#endif
#endif
...
void	usage(void) {
	puts("USAGE: main_tb [-cdpsth] <ZipElfProgram> <SDCardBackFile>\n"
"\n"
