...
// Evaluating clock hdmi_in_clk
	...
	virtual	void	sim_hdmi_in_clk_tick(void) {
		...
		// to this routine
		//
		writeout = (writeout)||(m_core->v__DOT__thehdmiin__DOT__copypix__DOT__frame_en_pipe);
		writeout = (writeout)||(m_core->v__DOT__vid_cyc);
		if (writeout) {
			//
			// SIM.DEBUG tags can print here, supporting
			// any attempts to debug by printf.  Following any
			...
			printf("%s%s %s%s @%08x ",
				m_core->v__DOT__vid_cyc?"CYC":"   ",
				m_core->v__DOT__vid_stb?"STB":"   ",
				m_core->v__DOT__vid_bus_ack?"ACK":"   ",
				m_core->v__DOT__vid_bus_stall?"STALL":"     ",
				m_core->v__DOT__vid_addr);
			for(int hdmiinj = 0; hdmiinj<4; hdmiinj++) {
				printf("%08x%c",
					m_core->v__DOT__vid_data[hdmiinj],
					(3==hdmiinj)?':':' ');
			}
			printf("\n");
		}
...
// Evaluating clock net_rx_clk
	...
	virtual	void	sim_net_rx_clk_tick(void) {
		...
		//
		// SIM.TICK from netp
		m_core->i_net_rx_ctl = m_core->o_net_tx_ctl;
