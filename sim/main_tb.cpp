...
public:
	...
	virtual	void	sim_clk_tick(void) {
		...
		(*m_sdram)(m_core->o_sdram_cyc,
			...
			m_core->o_sdram_we,
			m_core->o_sdram_addr,
			(uint32_t *)&m_core->o_sdram_data,
			m_core->o_sdram_sel,
			m_core->i_sdram_stall,
			m_core->i_sdram_ack,
			(uint32_t *)&m_core->i_sdram_data);
	...
	// Evaluating clock mclk

	// sim_mclk_tick() will be called from TESTB<Vmain>::tick()
	//   following any falling edge of clock mclk
	virtual	void	sim_mclk_tick(void) {
		//
		// SIM.TICK tags go here for SIM.CLOCK=mclk
		//
	...
	//
	// Step until clock mclk ticks
	//
	virtual	void	tick_mclk(void) {
		do {
			tick();
		} while(!m_mclk.rising_edge());
	}
