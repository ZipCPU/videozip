...
int	FLASHSIM::operator()(const int csn, const int sck, const int dat) {
	...
	if (csn) {
		...
		case QSPIF_PP: case QSPIF_PP4: case QSPIF_QPP: case QSPIF_QPP4:
			...
			break;
			// }}}
		case	QSPIF_SECTOR_ERASE: case QSPIF_SECTOR_ERASE4:
			// {{{
		...
		case	QSPIF_QUAD_READ_CMD:
			...
			break;
			// }}}
		case	QSPIF_QUAD_READ_CMD4:
			// {{{
			if ((m_mode_byte & 0x0f0)!=0x0a0)
				m_mode = FM_SPI;
			else
				m_state = QSPIF_QUAD_READ_IDLE4;
			break;
			// }}}
		case	QSPIF_DUAL_READ_CMD:
			...
			break;
			// }}}
		case	QSPIF_DUAL_READ_CMD4:
			// {{{
			if ((m_mode_byte & 0x0f0)!=0x0a0)
				m_mode = FM_SPI;
			else
				m_state = QSPIF_DUAL_READ_IDLE4;
			break;
			// }}}
		...
		case	QSPIF_DUAL_READ4:
			...
			break;
			// }}}
		case	QSPIF_DUAL_READ_IDLE:
		case	QSPIF_DUAL_READ_IDLE4:
		case	QSPIF_QUAD_READ_IDLE:
		case	QSPIF_QUAD_READ_IDLE4:
		default:
			break;
		}
	...
	if (m_state == QSPIF_QUAD_READ_IDLE) {
		...
		// }}}
	} else if (m_state == QSPIF_QUAD_READ_IDLE4) {
		// {{{
		// Cannot be in this state and deep power down
		assert((m_sreg & QSPIF_DEEP_POWER_DOWN_FLAG)==0);
		...
		assert(quad_mode());
		if (m_count == 32) {
			if (m_debug) printf("FLASHSIM: Entering from Quad-Read Idle4 to Quad-Read\n");
			if (m_debug) printf("FLASHSIM: QI/O Idle Addr = %02x\n", m_ireg&m_memmask);
			m_addr = (m_ireg) & m_memmask;
			assert((m_addr & (~(m_memmask)))==0);
			m_state = QSPIF_QUAD_READ4;
		} m_oreg = 0;
		// }}}
		...
		// }}}
	} else if (m_state == QSPIF_DUAL_READ_IDLE4) {
		// {{{
		// Cannot be in this state and deep power down
		assert((m_sreg & QSPIF_DEEP_POWER_DOWN_FLAG)==0);
		...
		case 0x06: // Write enable
			...
			break;
			// }}}
		case 0x0b: // Fast-Read (8 dummy bits)
			// {{{
			if (m_debug) printf("FLASHSIM: FAST-READ (single-bit)\n");
			m_state = QSPIF_FAST_READ;
			break;
			// }}}
		case 0x0c: // Fast-Read (8 dummy bits, 32 address bits)
			// {{{
			if (m_debug) printf("FLASHSIM: FAST-READ (single-bit)\n");
		...
		case 0x13: // Read data bytes, 4-byte address
			// {{{
			if (m_debug) printf("FLASHSIM: SLOW-READ (single-bit)\n");
			m_state = QSPIF_SLOW_READ4;
			break;
			// }}}
		case 0x30: // Clear status register
			// {{{
		...
		case 0x32: // QUAD Page program, 4 bits at a time
			...
			// }}}
		case 0x34: // QUAD Page program, 4 bits at a time, 32b address
			// {{{
			if (2 != (m_sreg & 0x203)) {
				if (m_debug) printf("FLASHSIM: Cannot program at this time, SREG = %x\n", m_sreg);
				m_state = QSPIF_INVALID;
		...
		case 0xbb: // Fast Read Dual I/O
			...
			break;
			// }}}
		case 0xbc: // Fast Read Dual I/O, 32b address
			// {{{
			// printf("QSPI: DUAL-I/O-READ\n");
			m_state = QSPIF_DUAL_READ_CMD4;
			m_mode = FM_DSPI;
			break;
			// }}}
		...
		case 0xdc: // Sector Erase, 32b address
			...
			if (2 != (m_sreg & 0x203)) {
				...
				m_state = QSPIF_SECTOR_ERASE4;
				if (m_debug) printf("FLASHSIM: SECTOR_ERASE COMMAND\n");
			} break;
			// }}}
		case 0x0eb: // Quad I/O read, 24b address
			// {{{
			...
			break;
			// }}}
		case 0x0ec: // Quad I/O read, 32b address
			// {{{
			// printf("QSPI: QUAD-I/O-READ\n");
			m_state = QSPIF_QUAD_READ_CMD4;
			m_mode = FM_QSPI;
			break;
			// }}}
		...
		case QSPIF_QUAD_READ_CMD4:
			...
			if (m_count == 40) {
				...
				m_mode_byte = (m_ireg) & 0x0ff;
				if (m_debug) printf("QSPI4: MODE BYTE = %02x\n", m_mode_byte);
			} else if ((m_count > 40+4*NDUMMY)&&(0 == (m_sreg&0x01))) {
				QOREG(m_mem[m_addr++]);
				// printf("QSPIF[%08x]/QR = %02x\n",
					// m_addr-1, m_oreg);
			} else m_oreg = 0;
		...
		case QSPIF_DUAL_READ4:
			...
			if (m_count == 40) {
				m_mode_byte = (m_ireg & 0x0ff);
				if (m_debug) printf("DSPI4/DR: MODE BYTE = %02x\n", m_mode_byte);
			}
			QOREG(m_mem[m_addr++]);
			if (m_debug) printf("DSPIF[%08x]/DR = %02x\n", m_addr-1, m_oreg & 0x0ff);
			break;
		...
		case QSPIF_SECTOR_ERASE:
			...
			// }}}
		case QSPIF_SECTOR_ERASE4:
			// {{{
			if (m_count == 40) {
				m_addr = m_ireg & (~0x3fff) & m_memmask;
				if (m_debug) printf("SECTOR_ERASE ADDRESS = %08x\n", m_addr);
			} break;
			// }}}
		case QSPIF_RELEASE:
