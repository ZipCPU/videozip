...
#ifdef	R_NETSCOPE
#define	SETSCOPE	m_fpga->writeio(R_NETSCOPE, 500)
#else
#define	SETSCOPE
#endif
...
"access\n\n");
	...
	if ((txstat & ENET_RESET) != 0)
		m_fpga->writeio(R_NET_TXCMD, (txstat &=(~ENET_RESET)));
	...
	if (config_hw_mac) {
		...
		{ unsigned cmd;
		cmd = ENET_TXGO|(ln<<2)|((config_hw_crc)?0:ENET_NOHWCRC);
