...
`define	MEGANETCPURX_ACCESS
`define	MEGANETCPUTX_ACCESS
`endif	// MEGANET_ACCESS
...
// }}}
	...
	wire		netbus_valid, netbus_ready, netbus_last;
	wire	[31:0]	netbus_pkdata;
	// wire	[1:0]	netbus_bytes;
...
// }}}
	...
	wire	[25:0]	wbwide_cpunetm_addr;
	wire	[127:0]	wbwide_cpunetm_data;
	wire	[15:0]	wbwide_cpunetm_sel;
	wire		wbwide_cpunetm_stall, wbwide_cpunetm_ack, wbwide_cpunetm_err;
	wire	[127:0]	wbwide_cpunetm_idata;
	// Verilator lint_on UNUSED
...
`endif
	...
	netdebug #(
		...
		// {{{
		.S_AXI_VALID(net_dbg_valid), .S_AXI_READY(net_dbg_ready),
			.S_AXI_DATA(net_dbg_data),
			.S_AXI_BYTES(net_dbg_bytes),
			.S_AXI_LAST(net_dbg_last),
		// }}}
		// Return network data
		// {{{
		.M_AXI_VALID(netbus_valid),
			.M_AXI_READY(netbus_ready),
			.M_AXI_DATA(netbus_pkdata),
			// BYTES=0, all words are full
			.M_AXI_LAST(netbus_last),
			// Not abortable here
		// }}}
...
`else	// NETBUS_ACCESS
	// {{{
	assign	netbus_valid = 1'b0;
	assign	netbus_pkdata  = 32'h0;
	// assign	netbus_bytes = 2'b0;
	assign	netbus_last  = 1'b1;
	assign	netbus_abort  = 1'b0;
	// Null bus master
	// {{{
	// }}}
	// }}}
...
`ifdef	CPUNET_ACCESS
	...
	cpunet #(
		...
		//
		.RX_VALID(netcpurx_valid),
		.RX_READY(netcpurx_ready),
		.RX_DATA( netcpurx_data),
		.RX_BYTES(netcpurx_bytes),
		.RX_LAST( netcpurx_last),
		.RX_ABORT(netcpurx_abort),
		//
		.TX_VALID(netcputx_valid),
		.TX_READY(netcputx_ready),
		.TX_DATA( netcputx_data),
		.TX_BYTES(netcputx_bytes),
		.TX_LAST( netcputx_last),
		.TX_ABORT(netcputx_abort),
		//
		.o_dma_cyc(wbwide_cpunetm_cyc), .o_dma_stb(wbwide_cpunetm_stb), .o_dma_we(wbwide_cpunetm_we),
			.o_dma_addr(wbwide_cpunetm_addr[26-1:0]),
			.o_dma_data(wbwide_cpunetm_data), // 128 bits wide
			.o_dma_sel(wbwide_cpunetm_sel),  // 128/8 bits wide
