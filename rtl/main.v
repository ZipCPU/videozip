...
// Unmet Dependency list for @$(PREFIX)=netclockctr
`ifdef	ETHERNET_ACCESS // This value is unknown
`ifdef	GPSTRK_ACCESS
`define	NETCLOCKCTR_ACCESS
`endif
`endif

// Unmet Dependency list for @$(PREFIX)=nettxctr
`ifdef	ETHERNET_ACCESS // This value is unknown
...
// }}}
	...
	4'hb: r_wb32_sio_data <= wb32_subseconds_idata;
	...
	assign	wb32_subseconds_stb = wb32_sio_stb && (wb32_sio_addr[ 3: 0] ==  4'hb);  // 0x02c
	...
	always @(posedge i_clk)
	casez(wb32_dio_addr[5:3])
	3'b000: r_wb32_dio_bus_select <= 3'd0;
	3'b001: r_wb32_dio_bus_select <= 3'd1;
	3'b010: r_wb32_dio_bus_select <= 3'd2;
	3'b011: r_wb32_dio_bus_select <= 3'd3;
	3'b100: r_wb32_dio_bus_select <= 3'd4;
	3'b11?: r_wb32_dio_bus_select <= 3'd5;
	default: r_wb32_dio_bus_select <= 0;
	endcase

	always	@(posedge i_clk)
	casez(r_wb32_dio_bus_select)
	3'd0: r_wb32_dio_data <= wb32_gck_idata;
	3'd1: r_wb32_dio_data <= wb32_i2cs_idata;
	3'd2: r_wb32_dio_data <= wb32_i2cdma_idata;
	3'd3: r_wb32_dio_data <= wb32_rtc_idata;
	3'd4: r_wb32_dio_data <= wb32_gtb_idata;
	3'd5: r_wb32_dio_data <= wb32_sio_idata;
	default: r_wb32_dio_data <= wb32_sio_idata;
	endcase

	assign	wb32_dio_idata = r_wb32_dio_data;

	assign	wb32_gck_cyc = wb32_dio_cyc;
	assign	wb32_gck_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h7) ==  3'h0);  // 0x000 - 0x00f
	...
	assign	wb32_i2cs_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h7) ==  3'h1);  // 0x020 - 0x02f
	...
	assign	wb32_i2cdma_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h7) ==  3'h2);  // 0x040 - 0x04f
	...
	assign	wb32_rtc_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h7) ==  3'h3);  // 0x060 - 0x06f
	...
	assign	wb32_gtb_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h7) ==  3'h4);  // 0x080 - 0x09f
	...
	assign	wb32_sio_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 3] &  3'h6) ==  3'h6);  // 0x0c0 - 0x0ff
	...
	// {{{
//
// Bus wbu has only one master (wbu) and one slave (wbu_zip)
// connected to it -- skipping the interconnect
//
	assign	wbu_zip_cyc  = wbu_cyc;
	assign	wbu_zip_stb  = wbu_stb;
	assign	wbu_zip_we   = wbu_we;
	assign	wbu_zip_addr = wbu_addr;
	assign	wbu_zip_data = wbu_data;
	assign	wbu_zip_sel  = wbu_sel;
	assign	wbu_zip_err = 1'b0;
	assign	wbu_err = wbu_zip_err; // Y
	assign	wbu_stall = wbu_zip_stall;
	assign	wbu_ack   = wbu_zip_ack;
	assign	wbu_idata = wbu_zip_idata;
...
`ifdef	GPSTRK_ACCESS
	gpsclock_tb #(.CLOCK_FREQUENCY_HZ(100000000))
		ppstb(i_clk, ck_pps, tb_pps,
			wb32_gtb_cyc, wb32_gtb_stb, wb32_gtb_we,
			wb32_gtb_addr[3-1:0],
			wb32_gtb_data, // 32 bits wide
			wb32_gtb_sel,  // 32/8 bits wide
		wb32_gtb_stall, wb32_gtb_ack, wb32_gtb_idata,
...
`ifdef	GPIO_ACCESS
	...
	//
	localparam [NGPO-1:0]	INITIAL_GPIO = 4'b1;
...
`ifdef	VERILATOR
	wire	verilator_halt;
	assign	o_trace = o_gpio[2];

	assign	verilator_halt = o_gpio[3];
...
`endif	// PWRCOUNT_ACCESS
	...
	begin
		r_buserr_addr <= 0;
		r_buserr_addr[7-1:0] <= wbu_addr[7-1:0];
	end
	assign	wb32_buserr_stall= 1'b0;
	assign	wb32_buserr_ack  = wb32_buserr_stb;
	assign	wb32_buserr_idata = { {(30-26){1'b0}},
			r_buserr_addr, 2'b00 };
`ifdef	ARBITRARY_CLOCK_GENERATOR_ACCESS
	// {{{
	////////////////////////////////////////////////////////////////////////
...
`ifdef	MEGANET_ACCESS
	...
	meganet #(
		...
		.o_ping_hwmac(net_last_ping_hwmac),
			...
			.o_net_adc_enable(net_adc_enable),
			.i_addr_switch({ 2'b0, wb_spio_idata[10:9] }),
		// Wishbone port
		// {{{
...
`ifdef	GPSTRK_ACCESS
	...
	gpsclock #(.DEFAULT_STEP(GPSCLOCK_DEFAULT_STEP))
		ppsck(i_clk, 1'b0, gps_pps, ck_pps, gps_led,
			wb32_gck_cyc, wb32_gck_stb, wb32_gck_we,
			wb32_gck_addr[2-1:0],
			wb32_gck_data, // 32 bits wide
			wb32_gck_sel,  // 32/8 bits wide
		wb32_gck_stall, wb32_gck_ack, wb32_gck_idata,
...
`ifdef	RTCDATE_ACCESS
	...
	rtcdate	rtcdatei(i_clk, rtc_ppd,
		wb32_rtcdate_cyc, wb32_rtcdate_stb, wb32_rtcdate_we,
			wb32_rtcdate_data, // 32 bits wide
			wb32_rtcdate_sel,  // 32/8 bits wide
		wb32_rtcdate_stall, wb32_rtcdate_ack, wb32_rtcdate_idata);
	// }}}
`else	// RTCDATE_ACCESS
	// {{{
	// }}}
...
`ifdef	GPSTRK_ACCESS
	rtcgps	#(RTC_CLKSTEP) thertc(i_clk, i_reset,
		wb32_rtc_cyc, wb32_rtc_stb, wb32_rtc_we,
			wb32_rtc_addr[2-1:0],
			wb32_rtc_data, // 32 bits wide
			wb32_rtc_sel,  // 32/8 bits wide
		wb32_rtc_stall, wb32_rtc_ack, wb32_rtc_idata,
		rtc_int, rtc_ppd,
