////////////////////////////////////////////////////////////////////////////////
//
// Filename:	rtl/ethernet/addemac.v
// {{{
...
module	addemac(
	...
	begin
		...
		if (i_en)
		begin
			// Add a H/W MAC to the incoming signal
			// {{{
			if ((!i_v)&&(!o_v))
			begin // No incoming signal
				// {{{
				o_v <= 1'b0;
				// }}}
			end else begin
...
`ifdef	FORMAL
	...
	if ((r_pos > 0)&&(!r_pos[4]))
	begin
		assert(f_cnt == r_pos-1'b1);
	end else if ((r_pos[4])&&(r_pos[3:0] != 0))
	begin
		assert(&f_cnt);
	end else if (r_pos == 0)
		assert(f_cnt == 0);
