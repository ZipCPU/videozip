////////////////////////////////////////////////////////////////////////////////
//
// Filename:	rtl/ethernet/pktgate.v
// {{{
...
module	pktgate #(
		...
		parameter [0:0]	OPT_DROP_ON_OVERFLOW = 1'b0,
		...
		input	wire [LGBYTES-1:0]	S_AXIN_BYTES,
		...
		output	wire [LGBYTES-1:0]	M_AXIN_BYTES,
	...
	if (!S_AXI_ARESETN)
		...
		if (!OPT_DROP_ON_OVERFLOW && output_active && !M_AXIN_VALID && s_midpacket)
			abort_incoming <= 1;
		if (OPT_DROP_ON_OVERFLOW && r_full && !lastv)
			abort_incoming <= 1;
...
`endif
	...
	begin
		...
		if (!M_AXIN_VALID && !OPT_DROP_ON_OVERFLOW)
			// Will never happen if we drop on overflow
			output_active <= 1'b0;
		...
		if (M_AXIN_ABORT && (!M_AXIN_VALID || M_AXIN_READY))
			output_active <= 1'b0;
	end else if (!M_AXIN_ABORT && (lastv
			|| (r_full && !S_AXIN_ABORT && !OPT_DROP_ON_OVERFLOW)))
		output_active <= 1'b1;
`ifdef	FORMAL
	always @(*)
