////////////////////////////////////////////////////////////////////////////////
//
// Filename:	rtl/video/pix2stream.v
// {{{
...
module	pix2stream #(
	...
	localparam	[1:0]		MODE_CLR8	= 2'b00,
					MODE_CLR16	= 2'b01,
					MODE_CLR24	= 2'b10,
					MODE_DIRECT	= 2'b11;
	...
	localparam	DW = BUS_DATA_WIDTH;	// Shorthand
	// Steps:
	//	t_	incoming data
	...
	reg	[DW-1:0]	clr8_word, clr8_zmask,
					clr16_word, clr16_zmask,
					clr_word, clr_zmask;

	reg	[$clog2(DW):0]	clr8_valid, clr16_valid, clr_valid, clr24_valid;
	reg	clr8_zvalid, clr16_zvalid, clr24_zvalid, clr_zvalid;
...
// `endif
	...
	always @(posedge i_clk)
	begin
		case({ skd_valid && skd_ready, z_step && (clr8_valid >= DW) })
		2'b01: clr8_valid <= clr8_valid - DW;
		2'b10: clr8_valid <= clr8_valid + 8;
		2'b11: clr8_valid <= clr8_valid + 8 - DW;
		default: begin end
		endcase
	...
	always @(posedge i_clk)
	if (z_step && clr8_valid >= DW)
	begin
		clr8_word <= {(DW){1'b0}};
		if (skd_valid && skd_ready)
			clr8_word[DW-1:DW-8]<= skd_clr8;
	end else if (skd_valid && skd_ready)
	begin
		clr8_word <= clr8_word
			| { skd_clr8, {(DW-8){1'b0}} } >> clr8_valid;
	end
	...
	if (i_reset)
		...
		clr8_zvalid <= (clr8_valid >= DW);

	assign	clr8_zfull = clr8_zvalid;
	// }}}
	...
	always @(posedge i_clk)
	begin
		case({ skd_valid && skd_ready, z_step && (clr16_valid >= DW) })
		2'b01: clr16_valid <= clr16_valid - DW;
		2'b10: clr16_valid <= clr16_valid + 16;
		2'b11: clr16_valid <= clr16_valid + 16 - DW;
		default: begin end
		endcase
	...
	always @(posedge i_clk)
	if (z_step && (clr16_valid >= DW))
	begin
		clr16_word <= {(DW){1'b0}};
		if (skd_valid && skd_ready)
			clr16_word[DW-1:DW-16] <= skd_clr16;
	end else if (skd_valid && skd_ready)
		clr16_word <= clr16_word
			| ({ skd_clr16,{(DW-16){1'b0}} } >> clr16_valid);

	always @(posedge i_clk)
	if (z_step && (clr16_valid >= DW))
		clr16_zmask <= clr16_word >> (DW-clr16_valid);
	...
	if (i_reset)
		...
		clr16_zvalid <= (clr16_valid >= DW);

	assign	clr16_zfull = clr16_zvalid;
	// }}}

	//
	// 24b color
	// {{{
	// clr24_valid is the *fill* in our 24b gearbox
	reg	[DW+24-1:0]	clr24_word;
	reg	[DW-1:0]	clr24_zmask;
	initial	clr24_valid = 0;
	always @(posedge i_clk)
	begin
		case({ skd_valid && skd_ready, z_step && (clr24_valid >= DW) })
		2'b01: clr24_valid <= clr24_valid - DW;
		2'b10: clr24_valid <= clr24_valid + 24;
		2'b11: clr24_valid <= clr24_valid + 24 - DW;
		default: begin end
		endcase

		if (z_step && clr_last)
			clr24_valid <= (skd_valid && skd_ready) ? 24: 0;
	...
	begin
		if (z_step && clr_last)
			clr24_word <= { skd_data, {(DW){1'b0}} };
		else if (z_step && clr24_valid >= DW)
			clr24_word <= (clr24_word << DW)
				|({skd_data, {(DW){1'b0}} } >> (clr24_valid-DW));
		else
			clr24_word <= clr24_word
				|({skd_data, {(DW){1'b0}} } >> clr24_valid);
	end else if (z_step && (clr_last || clr24_valid >= DW))
		clr24_word <= clr24_word << DW;

	always @(posedge i_clk)
	if (z_step && (clr24_valid >= DW || clr_last))
	begin
		clr24_zmask <= clr24_word[DW+24-1:24];
	end else if (z_step)
		clr24_zmask <= 0;

	initial	clr24_zvalid = 0;
	always @(posedge i_clk)
	if (i_reset)
		clr24_zvalid <= 0;
	else if (z_step)
		clr24_zvalid <= (clr24_valid >= DW) || clr_last;

	assign	clr24_zfull = clr24_zvalid;
	// }}}
	...
	always @(posedge i_clk)
	begin
		case({ skd_valid && skd_ready, z_step && (clr_valid >= DW) })
		2'b01: clr_valid <= clr_valid - DW;
		2'b10: clr_valid <= clr_valid + 32;
		2'b11: clr_valid <= clr_valid + 32 - DW;
		default: begin end
		endcase
	...
	begin : SAME_WIDTH
		...
		initial	clr_zmask = 0;
		always @(posedge i_clk)
		if (z_step || clr_valid >= DW || clr_last)
			clr_zmask <= clr_word;

	end else begin : PACK_32B
		initial	clr_word = 0;
		always @(posedge i_clk)
		if (z_step && (clr_valid >= DW || clr_last))
		begin
			...
			clr_word <= clr_word | ({ 8'h0, skd_data[23:0], {(DW-32){1'b0}} } >> clr_valid);
	...
	begin
		clr_zvalid <= 0;
		if (clr_valid >= DW || clr_last)
			clr_zvalid <= 1;
	end

	assign	clr_zfull = clr_zvalid;
	// }}}
	...
	begin
		...
		MODE_CLR24:	if (clr24_valid[LGDATA_WIDTH]) clr_full = 1;
	...
	begin
		...
		MODE_CLR24:	if (clr24_zfull) next_zvalid = 1;
	...
	MODE_CLR24:	mem_data <= clr24_zmask;
	...
	if (i_reset)
		...
		{ mem_user, mem_last } <= { clr_zuser, clr_zlast };
	// }}}
	////////////////////////////////////////////////////////////////////////
	//
	// Output generation
...
`ifdef	FORMAL
	...
	// {{{
	always @(*)
	for(ik=0; ik<DW; ik=ik+8)
	if (!i_reset && i_mode == 2'b01 && chk_neg_color && (clr8_valid + ik >= DW))
	...
	// {{{
	always @(*)
	for(ik=0; ik<DW; ik=ik+16)
	if (!i_reset && i_mode == 2'b10 && chk_neg_color && (clr16_valid + ik >= DW))
	...
	begin
		...
		if (!i_reset) case(i_mode)
		MODE_CLR8:	assert(clr8_valid  <= DW);
		MODE_CLR16:	assert(clr16_valid <= DW);
		MODE_CLR24:	begin end
		MODE_DIRECT:	assert(clr_valid   <= DW);
		endcase
		//
		...
		if (!i_reset) case(i_mode)
		MODE_CLR8:	assert(clr8_zvalid  <= DW);
		MODE_CLR16:	assert(clr16_zvalid <= DW);
		MODE_CLR24:	begin end
		MODE_DIRECT:	assert(clr_zvalid   <= DW);
		endcase
	end
	// }}}
